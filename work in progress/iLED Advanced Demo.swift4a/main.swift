...
var globalColor: RGBColor = RGBColor(0, 0, 0)
...
//-------------------------------------------------------------------------------
func iLEDColorWipeDynamic(pin: UInt8,
						count: UInt16, 
					 reverse: Bool = false, 
					grbOrder: Bool = true) {
	...
	if reverse {
		...
		for loop: UInt16 in 0...count {
			...
			while (index <= count) {
				iLEDWritePixel(pin: pin, color: RGBColor(0, 0, 0), grbOrder: grbOrder)          
				index = index &+ 1      
			}
...
func iLEDTheaterChase(pin: UInt8,
	...
	for _: UInt16 in 1...totalFrames { // Frame
		...
		for _: UInt16 in 1...count { // Pixel
  			...
  			if instruction.isOn {
				iLEDWritePixel(pin: pin, color: getRGBColor(), grbOrder: grbOrder)
    		}
  			else {
				iLEDWritePixel(pin: pin, color: RGBColor(0, 0, 0), grbOrder: grbOrder)
    		}
...
func simpleColorTest() {
	...
	iLEDShowColor(pin: rgbLEDPin, color: getRGBColor(), count: numberPixels)
}
...
func colorWipeTest() {
	...
	iLEDColorWipeDynamic(pin: rgbLEDPin, count: numberPixels)
	iLEDColorWipeDynamic(pin: rgbLEDPin, count: numberPixels, reverse: true)
}
