...
boolean MAX30105::begin(uint8_t i2caddr) {
  ...
  _sendBuffer(1000, "setup i2c", true); 
  ...
  if (readPartID() != MAX_30105_EXPECTEDPARTID) {
    ...
    _sendBuffer(1000, "readPartID fail", true);
    return false;
  }

  _sendBuffer(1000, "readPartID ok", true);

  // Populate revision ID
  readRevisionID();

  _sendBuffer(1000, "readRevisionID ok", true);
...
uint8_t MAX30105::readPartID() {
  _sendBuffer(1000, "reading part id", true); 
...
uint16_t MAX30105::check(void)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    _i2cWriteByte(MAX30105_ADDRESS, MAX30105_FIFODATA);
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      char * i2cBuffer = (char*)alloca(toGet);
      _i2cReadBuffer(MAX30105_ADDRESS, toGet, i2cBuffer);
      char i2cBufferPtr = 0;

      #define i2cReadByte i2cBuffer[i2cBufferPtr++]
...
uint8_t MAX30105::readRegister8(uint8_t address, uint8_t reg) {
  // _i2cWriteByte(address, reg);

  // _sendBuffer(1000, "reading register", true); 
  _i2cstart();
  // DEBUG_STRING("I2C:start sent");
  _i2cwrite((address&0x7f)<<1);
  // DEBUG_INT32(address);
  _i2cwrite(reg);
  // DEBUG_STRING("I2C:sent...");
  // DEBUG_INT32(reg);

  _i2cstart();
  _i2cwrite(((address&0x7f)<<1)+1);

  // receive one byte
  unsigned char byte = _i2cread(false);

  // uint8_t value = _i2cReadSingleByte(address);
  // DEBUG_STRING("I2C:got...");
  // DEBUG_INT32(value);
  _i2cstop();
  // DEBUG_STRING("I2C:stop sent");  
...
void MAX30105::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  ...
  _i2cWriteBuffer(address, 2, bytes, false);
